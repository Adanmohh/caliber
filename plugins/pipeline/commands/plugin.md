...
**Before starting:** Search memory for existing project context using `mcp__plugin_claude-mem_mcp-search__search`. Look for `[VALIDATE:]`, `[BUILD:]`, `[LAUNCH:]` tags with the project name.
...
After completion, output a clearly tagged summary (claude-mem auto-captures via PostToolUse hooks):
